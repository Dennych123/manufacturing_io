...
- `buffer-queue`: a feeder fills a buffer belt that stands still between demands, and one part
  is metered out at a time by running the belt until the exit beam has seen it pass. A
  queue-full beam throttles the feeder.
