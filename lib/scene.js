...
export function worldPoses(scene, dof) {
  ...
  for (const c of order) {
    ...
    const frame = compose(baseOf(c, defs, out), pose(c.at, c.rot));
...
/**
 * Components that are loose parts rather than machine: 'free' = a dynamic workpiece placed in
 * the scene. The plant makes them Rapier-dynamic bodies and streams their poses; they are not
 * machine links, so worldPoses() only gives their starting pose.
 * @param {Scene} scene @returns {Map<string, 'free'>}
 */
export function partRoles(scene) {
  /** @type {Map<string, 'free'>} */
  const m = new Map();
...
export function validate(scene) {
  ...
  for (const b of bindings(scene)) {
    ...
    (byTag.get(b.tag) || byTag.set(b.tag, []).get(b.tag)).push(b);
  }
...
/** Inline whatever fits in 100 columns, else one entry per line. @param {any} v @param {string} ind @param {number} [key] */
