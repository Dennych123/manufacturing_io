...
export function compile(scene) {
  ...
  const stat = new Map();
  /** Does this link of this component sit still for ever? @param {string} id @param {string} link */
...
export function validate(scene) {
  ...
  for (const c of scene.components) {
    ...
    for (const d of t.params) {
      ...
      else if (tgt.type !== d.of) e.push('component ' + c.id + ': params.' + d.k + ' must name a ' + d.of + ', not the ' + tgt.type + ' ' + v);
    }
  }
