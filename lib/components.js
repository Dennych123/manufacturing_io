...
const servoLinear = {
  ...
  step(/** @type {any} */ s, /** @type {any} */ p, /** @type {any} */ io, /** @type {number} */ dt) {
    ...
    if (!e && jp !== jn) {
      ...
      s.x = clamp(s.x + (jp ? 1 : -1) * p.vmax * ovrK(io.ovr) * dt, 0, p.stroke);
    }
    if (s.moving) {
...
/** Axis letters to unit vectors, for the generic joint. */
const AX = /** @type {Record<string, number[]>} */ ({ x: [1, 0, 0], y: [0, 1, 0], z: [0, 0, 1] });
...
const joint = {
  ...
  params: [
    ...
    { k: 'axis', type: 'enum', of: ['x', 'y', 'z'], def: 'x' },
  ...
  sockets: (/** @type {any} */ p) => ({
    ...
    end: { link: 'arm', at: p.kind === 'revolute' ? [0, p.len, 0] : [0, 0, 0] },
  ...
  shapes: (/** @type {any} */ p) => {
    ...
    if (p.kind === 'revolute') {
      ...
      const rot = p.axis === 'x' ? [0, 90, 0] : p.axis === 'y' ? [90, 0, 0] : [0, 0, 0];
      if (p.arm === 'plate') {
      ...
      s.push({ link: 'base', kind: 'cyl', r: w * 0.42, h: w * 0.7, at: [0, 0, 0], rot, mat: 'dark', collide: false });
      if (p.len > 0) s.push({ link: 'arm', kind: 'box', size: [w * 0.62, p.len, w * 0.62], at: [0, p.len / 2, 0], mat: 'alu', ...(p.color ? { color: p.color } : {}) });
    } else {
...
// A 2-position selector switch, AUTO / MANUAL, as on a Denso-style operator panel (rb4axis).
...
const workpiece = {
  ...
  check: (/** @type {any} */ p) => (p.size.every((/** @type {number} */ v) => v > 0) ? [] : ['size must be positive on every axis']),
};
...
const gripper = {
  ...
  shapes: (/** @type {any} */ p) => [
    ...
    { link: 'fingerL', kind: 'box', size: [p.fingerW * 2, p.fingerW, p.fingerLen], at: [0, -p.fingerW / 2, p.fingerLen / 2], mat: 'holder' },
    { link: 'fingerR', kind: 'box', size: [p.fingerW * 2, p.fingerW, p.fingerLen], at: [0, p.fingerW / 2, p.fingerLen / 2], mat: 'holder' },
  ],
...
export const TYPES = /** @type {Record<string, any>} */ ({ frame, plate, cylinder, servoLinear, pushbutton, selector, speedDial, lamp, workpiece, conveyor, emitter, remover, photoEye, proximity, vacuumCup, gripper, nest, indexTable, pallet, palletLift, joint });
