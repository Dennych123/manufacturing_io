...
const joint = {
  ...
  params: [
    ...
    { k: 'mesh', type: 'str', def: '' },
    // Mesh units to mm. A URDF is in metres, so 1000.
  ...
  shapes: (/** @type {any} */ p) => {
    ...
    if (p.kind === 'revolute') {
      ...
      if (p.mesh) s.push({ link: 'arm', kind: 'mesh', asset: p.mesh, scale: p.meshScale, at: [0, 0, 0], mat: 'alu', ...(p.color ? { color: p.color } : {}), collide: false });
