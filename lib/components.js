...
const gripper = {
  ...
  // plate it is bolted to at cell scale. Only the finger is a collider: the body and the slide
  // are `collide: false`, so the shape reads better and the physics is exactly what it was.
  shapes: (/** @type {any} */ p) => [
...
const shell = {
  ...
  shapes: (/** @type {any} */ p) => (p.asset
    ...
    : []),
  check: (/** @type {any} */ p) => (p.asset ? [] : ['asset must name a model under assets/']),
};
...
/** Relative luminance of a '#rrggbb' colour, 0..1. Below DARK a retro-reflective sensor is blind. */
...
export const TYPES = /** @type {Record<string, any>} */ ({ frame, plate, cylinder, servoLinear, pushbutton, selector, speedDial, lamp, workpiece, conveyor, emitter, remover, photoEye, proximity, vacuumCup, gripper, nest, indexTable, pallet, palletLift, joint, pitchBar, shell });
