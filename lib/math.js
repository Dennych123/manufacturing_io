...
export function qnorm(q) {
  // Math.hypot guards against overflow by rescaling, which costs several times a plain square
  // root. Quaternions and unit axes never come near the range where that matters.
...
export function qaxis(axis, deg) {
  const h = deg * DEG / 2, s = Math.sin(h), n = Math.sqrt(axis[0] * axis[0] + axis[1] * axis[1] + axis[2] * axis[2]) || 1;
...
/** Deterministic PRNG in [0, 1) (mulberry32), so jitter repeats run to run. @param {number} seed */
...
/** FNV-1a: a stable seed from a component id. @param {string} s */
